...
int main (void){
  ...
  while (y != height_shrunk) {
    while (x != width_shrunk) {
      ...
      brightness = ((img_resize[img_index + 0] * 0.2126) + (img_resize[img_index + 1] * 0.7152) + (img_resize[img_index + 2] * 0.0722));    //basically the entire ascii thing, so like till one y is done it keeps doing the x axis so like imagine (0,0) so it will keep increasing x by one until width is reached then it increases y by one and resets x. also it prints every pixel with a character based of brightness using luma formula
